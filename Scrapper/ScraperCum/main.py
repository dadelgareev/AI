...
class TsumScraper:
    ...
    def parse_count_pages(self, html, category):
        """
    ...
    def get_href_list(self, page=1, href_list=None):
        ...
        if href_list is None:
            href_list = []  # Инициализация пустого списка, если он не был передан
            ...
            for link_entry in unprocessed_links:
                ...
                for index, image_url in enumerate(image_urls):
                    ...
                    row_data = {
                        ...
                        'Id': result.get('article', "Артикул не найден"),
                        ...
                        'Tags': json.dumps(result.get("tags",[]), ensure_ascii=False)  # Конвертируем атрибуты в JSON
                    }
